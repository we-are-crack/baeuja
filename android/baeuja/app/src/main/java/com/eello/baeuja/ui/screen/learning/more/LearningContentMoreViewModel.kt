...
import com.eello.baeuja.ui.screen.learning.common.mapper.toLearningContentUiModel
...
class LearningContentMoreViewModel @Inject constructor(
    ...
    fun loadMoreItems(classification: Classification, size: Int = 10) {
        if (isLastPage.value) {
            Timber.Forest.d("더 이상 로드할 학습 콘텐츠가 없음.: page = $page, isLastPage = ${isLastPage.value}")
            return
        }

        Timber.Forest.d("학습 콘텐츠 로드: classification = $classification, page = $page")
        ...
        viewModelScope.launch {
            try {
                ...
                Timber.Forest.e(e, "학습 콘텐츠 로드 실패: ${e.message}")
            }
        }
